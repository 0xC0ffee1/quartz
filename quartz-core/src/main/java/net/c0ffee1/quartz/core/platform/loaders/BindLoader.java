...
public class BindLoader extends AbstractLoader<Class<?>> {
    public BindLoader(Reflections reflections) {
        super(reflections);
    }

    @Override
    public void load(Consumer<Class<?>> consumer) {
        getReflections().getTypesAnnotatedWith(Bind.class).forEach(consumer);
    }
}
